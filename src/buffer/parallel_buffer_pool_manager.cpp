...
ParallelBufferPoolManager::ParallelBufferPoolManager(size_t num_instances, size_t pool_size, DiskManager *disk_manager,
    ...
    : num_instances_(num_instances), pool_size_(pool_size), next_instance_(0) {
...
ParallelBufferPoolManager::~ParallelBufferPoolManager() {
  for (auto &buffer_pool_manager_instance : buffer_pool_managers_) {
    delete buffer_pool_manager_instance;
  }
}
...
auto ParallelBufferPoolManager::NewPgImp(page_id_t *page_id) -> Page * {
  ...
  // 1.   From a starting index of the BufferPoolInstances, call NewPageImpl until either 1) success and return 2)
  // looped around to starting index and return nullptr
  // 2.   Bump the starting index (mod number of instances) to start search at a different BPMI each time this function
  // is called
  size_t next_instance = next_instance_;
  do {
    Page *page = buffer_pool_managers_[next_instance]->NewPage(page_id);
    if (page != nullptr) {
      next_instance_ = (next_instance + 1) % num_instances_;
      return page;
    }
    next_instance = (next_instance + 1) % num_instances_;
  } while (next_instance != next_instance_);
  return nullptr;
}
...
void ParallelBufferPoolManager::FlushAllPgsImp() {
  ...
  for (auto &buffer_pool_manager_instance : buffer_pool_managers_) {
    buffer_pool_manager_instance->FlushAllPages();
  }
}
